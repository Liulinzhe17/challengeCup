...
- (void)loadThumWithContext:(CGContextRef)context {
    ...
    if (!self.isFirst) {
        ...
        //显示定位间隔偏好
        int index=0;
        if ([USER objectForKey:@"gatherTimes"]) {
            index=[[USER objectForKey:@"gatherTimes"]intValue];
...
- (void)loadSliderWithContext:(CGContextRef)context {
    ...
    for (int i = 0; i < self.numberOfPart; i++) {
        ...
        //        CGContextAddEllipseInRect(context, CGRectMake(x, y, partPointWidth, partPointHeight));//节点画成椭圆
        CGContextAddRect(context, CGRectMake(x, y, partPointWidth, partPointHeight));//节点画成矩形
        [self.partColor set];
        CGContextFillPath(context);
